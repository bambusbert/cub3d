...
static int	move_back(t_god *god)
{
	float	dx;
	float	dy;
...
static int	move_forward(t_god *god)
{
	float	dx;
	float	dy;
...
static int	move_left(t_god *god)
{
	float	dx;
	float	dy;
...
static int	move_right(t_god *god)
{
	float	dx;
	float	dy;
...
int	position_manager(t_god *god)
{
	...
	//	return (move_left_up(god));
	if (god->key_w)
		return (move_forward(god));
	if (god->key_s)
		return (move_back(god));
	if (god->key_a)
		return (move_left(god));
	if (god->key_d)
		return (move_right(god));
	return (0);
}
